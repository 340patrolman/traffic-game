...
TG.CONFIG = {
  ...
  AVENUE_V: [2], AVENUE_H: [2],   // 간선(왕복 4차로)으로 만들 도로 인덱스(가운데 남북·동서)
  ...
  CARS: {
    sedan: { id: 'sedan', name: '순찰 세단(중형)', w: 1.85, l: 4.7, maxSpeed: 36, accel: 7.0, brake: 8.5, latMax: 10.5,
             steerMax: 0.62, wheelbase: 2.75, grip: 9, revMax: 4.5, desc: '낮은 무게중심 · 코너 한계 높음' },
    suv:   { id: 'suv', name: '순찰 전기 SUV',  w: 1.95, l: 4.9, maxSpeed: 33, accel: 8.0, brake: 8.2, latMax: 8.8,
             steerMax: 0.58, wheelbase: 2.95,  grip: 8, revMax: 4.5, desc: '가속 빠름 · 무게중심 높아 코너에서 일찍 한계' },
    flag:  { id: 'flag', name: '순찰 대형 세단(플래그십)', w: 1.92, l: 5.05, maxSpeed: 40, accel: 7.6, brake: 8.8, latMax: 10.2,
             steerMax: 0.60, wheelbase: 3.05, grip: 9, revMax: 4.5, desc: '고해상도 차체 · 긴 축거로 안정, 회전 반경 큼' },
  },
  CAM_COCKPIT: { x: 0.38, y: 1.34, z: -0.02, fov: 90, lookDown: 0.09 },   // 실차 눈높이(≈1.3m). 계기판·핸들 윗부분이 시야 아래쪽에 들어온다   // 넓은 시야(양쪽 사이드미러·룸미러가 화면 안에 들어온다)   // 운전석(좌측 +x) 눈 위치 — 대시보드 위로 도로가 넓게 보이는 높이
