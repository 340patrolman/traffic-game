...
(function () {
  ...
  P.update = function (dt) {
    ...
    var sport = this.driveMode === 'sport', SP = (sport ? 1.28 : 1) * (this.boost || 1), MX = (sport && s.sportMax ? s.sportMax : s.maxSpeed) * (this.boostTop || 1);   // 스포츠 모드는 최고속도도 올라간다(추격·출동)   // 스포츠 모드: 가속·조향 응답·그립 한계가 조금씩 올라간다
    ...
    if (wantRev && vF < 0.6) {                       // 후진 버튼: 거의 정지면 즉시 후진
      ...
      else if (s.powertrain === 'ev') {   // 전기차: 출발부터 최대 가속(순간 토크) → evKnee 뒤로는 출력 일정(속도에 반비례) → 최고속도 가까이서 부드럽게 멈춘다
        var vp = Math.max(0, vF), kn = s.evKnee || 14, u = vp / MX;
