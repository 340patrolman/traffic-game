...
TG.vehmesh = (function () {
  ...
  var TYPES = {
    ...
    pickup: { w: 1.90, l: 5.25, belt: 1.00, wheelR: 0.38, taper: 0.04, pts: [[0.5, 0.45], [0.5, 0.86], [0.44, 0.98], [0.24, 1.06], [0.12, 1.74, 1], [-0.08, 1.78], [-0.12, 1.06, 1], [-0.5, 1.06], [-0.5, 0.45]] },   // 화물 번호판 픽업(개방형 적재함). 실존 차명은 쓰지 않는다
    ...
    // 🚑🚒🛻 현장에 오는 차들(v0.10.21) — 구급차(승합 기반 박스 차체) · 소방차(화물 기반) · 견인차(픽업 기반 + 붐).
    //  emer:true 면 지붕에 경광등 바가 서고, 그 램프는 traffic 이 깜빡인다. 제조사·소방서 표지는 그리지 않는다.
    amb:    { w: 2.02, l: 5.60, belt: 1.12, glassTop: 1.82, wheelR: 0.36, emer: true, pts: [[0.5, 0.44], [0.5, 0.96], [0.46, 1.06], [0.40, 2.16, 1], [-0.47, 2.22], [-0.5, 2.10], [-0.5, 0.44]] },
    fire:   { w: 2.30, l: 7.20, belt: 1.18, glassTop: 2.10, wheelR: 0.44, cargo: true, emer: true, pts: [[0.5, 0.52], [0.5, 1.16], [0.47, 1.34], [0.42, 2.45, 1], [0.10, 2.52], [0.08, 1.30], [0.08, 0.52]] },
    tow:    { w: 1.96, l: 5.90, belt: 1.02, wheelR: 0.39, taper: 0.04, tow: true, emer: true, pts: [[0.5, 0.46], [0.5, 0.90], [0.44, 1.02], [0.24, 1.10], [0.12, 1.86, 1], [-0.08, 1.90], [-0.12, 1.10, 1], [-0.5, 1.10], [-0.5, 0.46]] },
    ...
    psuv:   { w: 1.89, l: 4.65, belt: 1.02, wheelR: 0.36, wb: 3.0, police: true, detail: true, taper: 0.03, roofScale: 0.90, pixel: true, tailBar: true,
    ...
    // 순찰 수소 SUV: 전기 SUV 보다 지붕이 둥글고 뒤가 길다(픽셀 LED 는 쓰지 않는다)
    phsuv:  { w: 1.87, l: 4.67, belt: 1.00, wheelR: 0.35, wb: 2.79, police: true, detail: true, taper: 0.05, roofScale: 0.88, tailBar: true,
              pts: [[0.5, 0.42], [0.5, 0.62], [0.48, 0.80], [0.44, 0.90], [0.36, 0.96], [0.22, 1.00], [0.12, 1.03], [0.01, 1.50, 1], [-0.12, 1.58], [-0.30, 1.58], [-0.42, 1.44, 1], [-0.47, 1.10], [-0.5, 0.92], [-0.5, 0.42]] },
    // 순찰 준대형 세단: 중형과 플래그십 사이 — 보닛이 길고 지붕선이 완만하다
    pgsedan:{ w: 1.89, l: 4.99, belt: 0.94, wheelR: 0.35, wb: 2.9, police: true, detail: true, taper: 0.065, roofScale: 0.85, tailBar: true,
              pts: [[0.5, 0.37], [0.5, 0.51], [0.49, 0.64], [0.465, 0.72], [0.41, 0.77], [0.29, 0.82], [0.18, 0.86], [0.11, 0.89], [0.025, 1.21, 1], [-0.065, 1.35, 1], [-0.14, 1.41], [-0.245, 1.44], [-0.40, 1.40], [-0.47, 1.16], [-0.5, 0.96], [-0.5, 0.37]] },
  ...
  function body(gb, T, color, opts) {
    ...
    // 경찰차: 경광등 받침(지붕 최고점)
    if (T.police || T.emer) { var rY = roofY(T); gb.box(0, rY + 0.03, -l * 0.04, 1.05, 0.06, 0.36, 0x2b2f35, {}); }
    // 견인차: 뒤쪽 붐(기둥 + 팔) — 소유자 현장 자료의 견인 절차와 같은 장비다
    if (T.tow) {
      gb.box(0, 1.55, -l * 0.10, 0.22, 0.9, 0.22, 0x3a4149, {});
      gb.box(0, 1.92, -l * 0.28, 0.18, 0.18, l * 0.44, 0x6b7480, {});
    }
    // 버스: 앞 행선판 + 문
    if (T.bus) {
  ...
  function interior(T) {
    ...
    gb.box(ox, belt - 0.02, oz, 0.42, 0.56, 0.26, 0xf2f4f7, {});                                     // 상의(교통경찰 근무복 — 흰색)
    ...
    gb.box(ox + 0.26, belt - 0.10, oz + 0.10, 0.10, 0.44, 0.12, 0xf2f4f7, {}); gb.box(ox - 0.26, belt - 0.10, oz + 0.10, 0.10, 0.44, 0.12, 0xf2f4f7, {});   // 팔(흰 소매)
  ...
  return { TYPES: TYPES, roofY: roofY, build: build, glass: glass, driver: driver, wheelGeo: wheelGeo, interior: interior, roofY: roofY, layout: layout, hoodAt: hoodAt, profile: profile, hoodDecal: hoodDecal, sideDecal: sideDecal };
})();
